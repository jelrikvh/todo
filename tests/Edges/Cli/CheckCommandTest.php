...
/**
 * @covers \Todo\Edges\Cli\CheckCommand
...
final class CheckCommandTest extends KernelTestCase
{
    ...
    public function test_it_checks_a_previously_unchecked_item(): void
    {
        (new FileSystem())->copy(
...
TEXT
    ...
    public function test_it_does_not_fail_when_the_item_is_already_checked(): void
    {
        (new FileSystem())->copy(
            sprintf('%s/todolist-for-tests', __DIR__),
            sprintf('%s/../../.data/test/list', __DIR__)
        ...
        $this->assertStringContainsString(
            '2: [x]',
            $output,
            "The third item was expected to be checked when the test starts; it isn't."
        );
