...
/**
 * @covers \Todo\Edges\Cli\UpCommand
 * @covers \Todo\Edges\Cli\DisplayHelper
 */
final class UpCommandTest extends KernelTestCase
{
    ...
    public function test_it_moves_an_item_up(): void
    {
        ...
        $command = $application->find('todo:up');
        $commandTester = new CommandTester($command);

        $commandTester->execute(['itemNumber' => 2]);
...
TEXT
    ...
    public function test_it_does_not_fail_when_the_item_is_already_at_the_top_of_the_list(): void
    {
        (new FileSystem())->copy(
            sprintf('%s/todolist-for-tests', __DIR__),
            sprintf('%s/../../.data/test/list', __DIR__)
        ...
        $this->assertStringContainsString(
            '0: [ ] Item 1, unchecked',
            $output,
            "The first item was expected to be at the top already when the test starts; it isn't."
        );
